...
struct TypeTable *createNewTypeForTuple(struct TupleType *tupleType) {
    struct TypeTable *type = createNewType(tupleType->name);
    struct TupleField *tupleField = tupleType->fields;

    struct TypeTable *head = NULL;
    struct TypeTable *tail = NULL;
    while (tupleField) {
        // struct FieldList *field = fieldList
        struct TypeTable *fieldType = typeTableLookup(tupleField->name);
        if (!head) {
            head = fieldType;
            tail = fieldType;
        } else {
