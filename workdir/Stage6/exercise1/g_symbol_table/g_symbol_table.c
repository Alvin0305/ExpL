...
struct GSymbol *installGST(char *name, enum Type type, int size, bool isPtr, struct TupleType *tupleType, struct TypeTable *typeTableEntry, int functionLabel, bool isArray) {
    ...
    printf("installing with size: %d\n", size);

    if (lookupGST(name)) {
        compilerError(E_VARIABLE_REDECLARATION, name);
    }

    struct GSymbol *newEntry = createSymbolTableEntry(name, type, size, isPtr, functionLabel, isArray);
...
void addToGST(struct tnode *typeNode, struct tnode *varListNode) {
    printf("type %d %p\n", typeNode->type, typeNode->typeTableEntry);
...
static void populateGST(struct tnode *root) {
    ...
    switch (root->nodeType) {
        ...
        case NODE_VARIABLE:
            ...
            size = currentDataType == TUPLE ? currentTupleType->size : currentDataType == USER_TYPE ? getSizeOfType(currentUserDefinedType)
                                                                                                    : getSizeOfDataType(currentDataType);

            entry = installGST(root->varName, currentDataType, size, false, currentTupleType, currentUserDefinedType, NO_LABEL, false);
        ...
        case NODE_DEREFERENCE:
            ...
            size = currentDataType == TUPLE ? currentTupleType->size : currentDataType == USER_TYPE ? getSizeOfType(currentUserDefinedType)
                                                                                                    : getSizeOfDataType(currentDataType);
            entry = installGST(idNode->varName, currentDataType, size, true, currentTupleType, currentUserDefinedType, NO_LABEL, false);
        ...
        case NODE_FUNC_DECL:
            ...
            entry = installGST(idNode->varName, currentDataType, getSizeOfDataType(currentDataType), false, currentTupleType, currentUserDefinedType, createNewLabel(), false);
...
static bool checkParamsForFunction(struct tnode *node, struct Param **param, char *functionName) {
    ...
    switch (node->nodeType) {
        ...
        case NODE_USER_DEF_TYPE_PARAM:
            ...
            if (!(*param)->typeTableEntry) {
                printf("[ERROR]: Param is User Defined type, but does not have a type table entry\n");
            }

            bool typeMatch = (((*param)->type == USER_TYPE) && (strcmp(typeTableEntry->name, (*param)->typeTableEntry->name) == 0));
            bool nameMatch = (strcmp(idNode->varName, (*param)->name) == 0);
...
static int computerArraySize(struct tnode *dimensionDeclNode, int *dimensionCount) {
    if (currentDataType == USER_TYPE) {
        return computeDimensionProduct(dimensionDeclNode, dimensionCount) * getSizeOfType(currentUserDefinedType);
    } else {
        printf("problem...");
