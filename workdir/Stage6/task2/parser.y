...
%type <node> globalDeclarationsBlock globalDeclarationList globalDecl globalTupleDeclaration globalVarList
%type <node> functionDefinitionBlock functionDefinition functionCall argList
...
%type <node> statementList statement assignmentStatement compoundAssignment
%type <node> whileStatement ifStatement repeatUntilStatement doWhileStatement
%type <node> inputStatement outputStatement allocStatement freeStatement
%type <node> localDeclBlock localDeclList localDecl localTupleDecl idList 
%type <node> type
...
%token READ WRITE KW_ALLOC KW_FREE
...
statement : inputStatement                                  { $$ = $1; }
    ...
    | allocStatement SEMI                                   { $$ = $1; }
    | freeStatement SEMI                                    { $$ = $1; }
    ;

allocStatement : ID ASSIGN KW_ALLOC '(' ')'                 { $$ = createAllocNode($1); }
    | memberAccess ASSIGN KW_ALLOC '(' ')'                  { $$ = createAllocNode($1); }
    ;

freeStatement : KW_FREE '(' ID ')'                          { $$ = createFreeNode($3); }
    ;
