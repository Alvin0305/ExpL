...
#include "../../util/util.h"
#include "../member_access/node_member_access.h"
...
struct Node *createReadToMemberNode(struct Node *memberNode) {
    struct Node *node = createEmptyNode();
    struct Node *newIdNode = createMemberAccessNode(memberNode->left, memberNode->right, ACCESS_DOT, false, NULL);

    node->left = newIdNode;
    node->right = NULL;
    node->nodeType = NODE_READ_TO_MEMBER;
    node->numVal = __NONE__;
    node->type = NONE;
    node->varName = NULL;
