...
void generateConditionCode(struct Node *node, int trueLabel, int falseLabel) {
    ...
    switch (node->nodeType) {
        ...
        default:
            printf("Unhandled condition node: ");
            printNode(node);
            break;
    }
...
void generateIfElseCode(struct Node *node) {
    ...
    struct Node *conditionNode = node->left;
    struct Node *connectorNode = node->right;
    struct Node *ifNode = connectorNode->left;
    struct Node *elseNode = connectorNode->right;
