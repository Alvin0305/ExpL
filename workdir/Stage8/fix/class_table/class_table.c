...
static struct ClassMethod *mergeParentChildMethods(char *childClassName, struct ClassMethod *parentMethods, struct ClassMethod *childMethods) {
    ...
    if (numMethods > MAX_NUM_METHODS) {
        compilerError(E_INVALID_SIZE_FOR_CLASS, childClassName);
    }
...
void setMethodsToClass(struct ClassMethod *methods) {
    activeClass->methods = mergeParentChildMethods(activeClass->name, activeClass->methods, methods);
    activeClass->numMethods = numMethods;

    struct ClassMethod *curr = activeClass->methods;
    int index = 0;
    while (curr) {
        curr->methodIndex = index++;
        curr = curr->next;
    }

    stackTop += MAX_NUM_METHODS;
}
...
void checkMethodSignature(struct TypeInfo *returnType, char *methodName, struct Param *params) {
    ...
    if (!method) {
        compilerError(E_FUNCTION_USED_BEFORE_DECLARATION, methodName, getParamTypesFromParams(params), getNumOfParams(params));
    }
