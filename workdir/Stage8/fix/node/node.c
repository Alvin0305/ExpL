...
struct Node *createVariableUsageNode(char *varName) {
    ...
    GSymbol *globalEntry = lookupGST(varName, false, NULL, 0);
...
struct Node *createNewNode(struct Node *leftNode, struct Node *classNameNode) {
    ...
    if (!_class) {
        compilerError(E_CLASS_USED_BEFORE_DECLARATION, classNameNode->varName);
    }

    struct Node *node;
    if (leftNode->nodeType == NODE_VARIABLE) {
        struct Node *varUsageNode = createVariableUsageNode(leftNode->varName);
        if (!isTypeCompatible(varUsageNode->typeInfo, createTypeInfo(CLASS, NULL, NULL, _class))) {
        ...
        if (!isTypeCompatible(leftNode->typeInfo, createTypeInfo(CLASS, NULL, NULL, _class))) {
            printf("type mismatch in assignment\n");
            exit(1);
        }
