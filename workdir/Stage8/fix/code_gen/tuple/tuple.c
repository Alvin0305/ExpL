...
typedef enum { SCOPE_LOCAL, SCOPE_GLOBAL, SCOPE_NONE } Scope;
...
static struct TupleType *getTupleTypeFromST(char *tupleTypeName) {
    ...
    GSymbol *globalEntry = lookupGST(tupleTypeName, false, NULL, 0);

    if (localEntry) {
        scope = SCOPE_LOCAL;
        return localEntry->typeInfo->tupleType;
    } else if (globalEntry) {
        scope = SCOPE_GLOBAL;
        return globalEntry->typeInfo->tupleType;
    } else {
        scope = SCOPE_NONE;
...
int generateGetTupleMemoryAddrCode(struct Node *tupleAccessNode) {
    struct Node *tupleIdNode = tupleAccessNode->left;
    struct Node *tupleFieldNode = tupleAccessNode->right;
...
int generateGetTuplePointerMemoryAddrCode(struct Node *tuplePointerAccessNode) {
    struct Node *tupleIdNode = tuplePointerAccessNode->left;
    struct Node *tupleFieldNode = tuplePointerAccessNode->right;
...
void generateTupleAssignCode(struct Node *node) {
    struct Node *tupleAccessNode = node->left;
    struct Node *exprNode = node->right;
