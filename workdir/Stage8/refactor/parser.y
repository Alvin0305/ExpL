...
%token KW_READ KW_WRITE KW_ALLOC KW_FREE KW_INIT KW_NEW
%token PLUS MUL MINUS DIV MOD 
%token ASSIGN 
%token COMMA SEMI DOT ARROW AMPERSAND
...
%token KW_RETURN KW_MAIN
%token KW_TYPE KW_ENDTYPE
%token KW_NULL KW_BRKP
%token KW_CLASS KW_ENDCLASS KW_EXTENDS
...
className : ID                                                      { createNewClass($1->varName, NULL); }
    | ID KW_EXTENDS ID                                              { createNewClass($1->varName, $3->varName); }
    ;

classFieldsDeclBlock : KW_DECL classFieldsDeclList 
            classMethodsDeclList KW_ENDDECL                         { setFieldsToClass($2); setMethodsToClass($3); }
    | KW_DECL classFieldsDeclList KW_ENDDECL                        { setFieldsToClass($2); setMethodsToClass(NULL); }
    | KW_DECL classMethodsDeclList KW_ENDDECL                       { setFieldsToClass(NULL); setMethodsToClass($2); }
    | KW_DECL KW_ENDDECL                                            { setFieldsToClass(NULL); setMethodsToClass(NULL); }
    |
    ;
...
globalDecl : type globalVarList SEMI                                { populateGST($1, $2); }    
...
globalTupleDecl : KW_TUPLE ID                                       { createNewTupleType($2->varName); } 
    ...
    tupleIdList                                                     { populateGST(
                                                                        createTypeInfo(
                                                                            TUPLE, 
                                                                            lookupTupleTypeTable($2->varName), 
                                                                            lookupTT($2->varName), 
                                                                            NULL), 
                                                                        $8); }
    ;
...
type : TYPE_INT                                                     { $$ = createTypeInfo(INT, NULL, lookupTT("INT"), NULL); }
    ...
    | KW_TUPLE ID                                                   { $$ = createTypeInfo(TUPLE, 
                                                                      lookupTupleTypeTable($2->varName), NULL, NULL); }
    | TYPE_VOID                                                     { $$ = createTypeInfo(VOID, NULL, lookupTT("VOID"), NULL); }
    | ID                                                            { $$ = createTypeInfo(UNKNOWN, NULL, 
                                                                      lookupTT($1->varName), lookupCT($1->varName)); }
    ;
...
funcDef : type ID                                                   
    '(' paramList ')'                                               { currentFunction = lookupGST($2->varName, 
                                                                                                    true, 
                                                                                                    getParamTypesFromParams($4), 
                                                                                                    getNumOfParams($4)
                                                                                                ); }
    '{' localDeclBlock body '}'                                     { checkFunctionSignature($1, $2->varName, $4);
...
localTupleDecl : KW_TUPLE ID                                        { createNewTupleType($2->varName); } 
        '(' tupleFieldList ')'                                      { setFieldsToTupleType($2->varName, $5); }
        tupleIdList SEMI                                            { installVarsToLST(
                                                                        createTypeInfo(
                                                                            TUPLE, 
                                                                            lookupTupleTypeTable($2->varName), 
                                                                            lookupTT($2->varName), 
...
mainBlock : TYPE_INT KW_MAIN '(' ')' '{' localDeclBlock body '}'    { generateMainCode($7); 
                                                                      freeLocalSymbolTable(); }
    ;
...
inputStmt : KW_READ '(' ID ')' SEMI                                 { $$ = createReadNode($3); }
    ...
    | KW_READ '(' memberAccess ')' SEMI                             { $$ = createReadToMemberNode($3); }
    ;
...
ifStmt : KW_IF '(' boolexpr ')' KW_THEN stmtList KW_ELSE 
        stmtList KW_ENDIF                                           { $$ = createIfElseNode($3, $6, $8); }
