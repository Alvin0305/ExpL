...
static void collectArrayIndexRegisters(struct Node *node, int *indexRegisters, int *count) {
    if (!node) return;
...
int generateArrayElementAddress(struct Node *arrayAccessNode) {
    struct Node *idNode = arrayAccessNode->left;
    ...
    int size = sizeOfTypeOnStack(entry->typeInfo, NULL, false, false);
