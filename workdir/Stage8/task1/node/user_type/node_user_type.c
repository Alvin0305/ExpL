...
struct Node *createMemberAccessNode(struct Node *baseExprNode, struct Node *memberIdNode, AccessType accessType,
    ...
    static struct TypeField *resolvedTypeField = NULL;
            ...
            switch (baseIdNode->type) {
                ...
                case CLASS:
                    if (isMethodCall) {
                        ...
                        struct Param *paramIter = method->params->next;
                        checkArgs(&paramIter, argList, memberIdNode->varName);
                        if (paramIter) {
                            compilerError(E_FUNCTION_CALL_TOO_FEW_ARG, memberIdNode->varName);
                        }
