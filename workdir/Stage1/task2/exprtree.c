...
void prefix_helper(struct tnode *root) {
    if (root) {
        if (!root->op) {
            printf("%d ", root->val);
            return;
        ...
        prefix_helper(root->left);
        prefix_helper(root->right);
    }
}
...
void postfix_helper(struct tnode *root) {
    if (root) {
        postfix_helper(root->left);
        postfix_helper(root->right);
