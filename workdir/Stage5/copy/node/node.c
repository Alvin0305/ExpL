...
struct tnode *createTupleParamNode(struct tnode *tupleTypeNameNode, struct tnode *idNode, bool addToLocalSymbolTable, bool isPtr) {
    ...
    if (addToLocalSymbolTable) {
        local_st_tuple_install(idNode->varName, tupleType, isPtr, true);
    }

    struct tnode *node = createConnectorNode(tupleTypeNameNode, idNode);

    if (isPtr) {
        node->nodeType = NODE_TUPLE_POINTER_PARAM;
    } else {
        node->nodeType = NODE_TUPLE_PARAM;
    }
...
struct tnode *createGlobalTupleDeclarationNode(struct tnode *tupleTypeNameNode, struct tnode *idListNode) {
    struct tnode *node = createConnectorNode(tupleTypeNameNode, idListNode);
...
struct tnode *createLocalTupleDeclNode(struct tnode *tupleTypeNameNode, struct tnode *idListNode) {
    struct TupleType *tupleType = lookupTupleTypeTable(tupleTypeNameNode->varName);
    if (!tupleType) {
        compilerError(E_TUPLE_TYPE_USED_BEFORE_DECLARATION, tupleTypeNameNode->varName);
    }
