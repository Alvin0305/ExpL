...
%type <node> globalTupleDeclaration tupleParamList tupleIdList tupleParam localTupleDecl tupleAssignmentStatement tupleAccess
...
globalDeclarationsBlock : KW_DECL globalDeclarationList KW_ENDDECL { 
    ...
    | KW_DECL KW_ENDDECL { $$ = NULL; generateHeader(); }
    ;

globalDeclarationList : globalDeclarationList globalDecl    { $$ = createConnectorNode($1, $2); }
    | globalDecl                                            { $$ = $1; }
    ;

globalDecl : type globalVarList SEMI    { $$ = createDeclarationNode($1, $2); }
    | globalTupleDeclaration            { $$ = $1; }
    ;

globalVarList : globalVarList COMMA ID                      { $$ = createConnectorNode($1, $3); }
    | globalVarList COMMA ID dimensionDecl                  { $$ = createNewArrayNode($1, $3, $4); }
...
globalTupleDeclaration : KW_TUPLE ID '(' tupleParamList ')' tupleIdList SEMI        { $$ = createGlobalTupleTypeNode($2, $4, $6); }
    | KW_TUPLE ID tupleIdList SEMI                                                  { $$ = createGlobalTupleDeclarationNode($2, $3); }
    ;
...
tupleParam : type ID    { $$ = createConnectorNode($1, $2); }
    ;
...
funcDeclParam : type ID     { $$ = createParamNode($1, $2, false); }
    | type MUL ID           { $$ = createPointerParamNode($1, $3, false); }
    | KW_TUPLE ID ID        { $$ = createTupleParamNode($2, $3, false, false); }
    | KW_TUPLE ID MUL ID    { $$ = createTupleParamNode($2, $4, false, true); }
    ;
...
param : type ID                         { $$ = createParamNode($1, $2, true); }
    | type MUL ID                       { $$ = createPointerParamNode($1, $3, true); }
    | KW_TUPLE ID ID                    { $$ = createTupleParamNode($2, $3, true, false); }
    | KW_TUPLE ID MUL ID                { $$ = createTupleParamNode($2, $4, true, true); }
    ;
...
localTupleDecl : KW_TUPLE ID '(' tupleParamList ')' tupleIdList SEMI        { $$ = createLocalTupleTypeDeclarationNode($2, $4, $6); }
    | KW_TUPLE ID tupleIdList SEMI                                          { $$ = createLocalTupleDeclNode($2, $3); }
    ;
...
mainBlock : TYPE_INT MAIN '(' ')' '{' localDeclBlock body '}' { $$ = createMainNode($6, $7);     printf("Hello\n"); // Add \n
