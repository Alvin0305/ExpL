...
struct tnode *createPointerParamNode(struct tnode *typeNode, struct tnode *idNode, bool addToLocalSymbolTable) {
    ...
    if (addToLocalSymbolTable) {
        local_st_install(idNode->varName, typeNode->type, true, true);
    }

    node->nodeType = NODE_POINTER_PARAM;

    return node;
}

bool checkArgs(Param **param, struct tnode *argListNode) {
    if (!argListNode && !*param)
        return true;
    if (!argListNode || !*param)
        throwFunctionCallError(E_FUNCTION_CALL_MORE_ARG);

    switch (argListNode->nodeType) {
    case NODE_CONNECTOR:
        bool leftOK = checkArgs(param, argListNode->left);
        bool rightOK = checkArgs(param, argListNode->right);
        return leftOK && rightOK;

    default:
        printf("checking %s %s %s\n", (*param)->name, dataTypeToString((*param)->type), dataTypeToString(argListNode->type));
...
struct tnode *createFunctionCallNode(struct tnode *functionNameNode, struct tnode *argListNode) {
    ...
    bool check = checkArgs(&paramIter, argListNode);
    if (check && paramIter) {
        throwFunctionCallError(E_FUNCTION_CALL_TOO_FEW_ARG);
    }
...
struct tnode *createFunctionDefinitionNode(struct tnode *returnTypeNode, struct tnode *functionNameNode, struct tnode *paramListNode,
    ...
    printf("got function declaration for %s\n", functionNameNode->varName);
...
struct tnode *createBreakPointNode() {
    ...
    node->nodeType = NODE_BREAK_POINT;
...
struct tnode *createLocalTupleTypeDeclarationNode(struct tnode *idNode, struct tnode *paramListNode, struct tnode *idListNode) {
    ...
    struct Tuple *tupleType = createTupleFromNode(idNode->varName, paramListNode);
