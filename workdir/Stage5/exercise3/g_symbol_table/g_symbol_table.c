...
struct TupleType *currentTupleType = NULL;
...
void populateGST(struct tnode *root) {
    ...
    case NODE_FUNC_DECL: {
        ...
        entry = install(idNode->varName, currentDataType, getSizeOfDataType(currentDataType), false, currentTupleType);
    ...
    case NODE_TUPLE_VAR_DECL: {
        if (root->right && root->right->nodeType == NODE_FUNC_DECL) {
            struct tnode *idNode = root->right->left;
            struct tnode *paramListNode = root->right->right;
            ...
            entry = install_gt_tuple(idNode->varName, tupleType, false);
            entry->functionLabel = createNewLabel();
            addParamsToFunction(entry, paramListNode);
            break;
        }
...
void addParamsToFunction(struct GSymbol *entry, struct tnode *paramListNode) {
    ...
    if (paramListNode->nodeType == NODE_CONNECTOR) {
        ...
        if (!tupleType) {
            printf("[ERROR]: tuppe tpye is null\n");
            // compilerError(E_TUPLE_TYPE_USED_BEFORE_DECLARATION, tupleTypeNameNode->varName);
        }
...
bool checkParamsForFunction(struct tnode *node, struct Param **param, char *functionName) {
    ...
    if (node->nodeType == NODE_PARAM || node->nodeType == NODE_POINTER_PARAM) {
        ...
        if (typeNode->type == TUPLE) {
            typeMatch = typeMatch && (strcmp(typeNode->tupleType->name, (*param)->tupleType->name) == 0);
        }
