...
%type <node> functionDefinitionBlock functionDefinition localDeclBlock localDeclList localDecl idList
...
globalVarList : globalVarList COMMA ID              { $$ = createConnectorNode($1, $3); }
    ...
    | globalVarList COMMA ID '(' paramList ')'      { $$ = createConnectorNode($1, createFunctionDeclarationNode($3, $5)); }
    | ID dimensionDecl                              { $$ = createNewArrayNode(NULL, $1, $2); }
    | MUL ID                                        { $$ = createNewPointerNode($2); }
    | ID                                            { $$ = $1; }
    | ID '(' paramList ')'                          { $$ = createFunctionDeclarationNode($1, $3); }
    ;
...
functionDefinition : type ID '(' paramList ')' '{' localDeclBlock body '}'  { $$ = createFunctionDefinitionNode($1, $2, $4, $7, $8); }
    ;
...
param : type ID                         { $$ = createParamNode($1, $2); }
    ;
...
localDecl : type idList SEMI                            { $$ = NULL; }
    ;

idList : idList COMMA ID                                { $$ = NULL; }
    | ID
    ;
...
mainBlock : KW_BEGIN statementList KW_END { $$ = $2; }
    ;
