...
struct tnode *createVariableUsageNode(char *varName) {
    ...
    if (localEntry) {
        ...
        node->typeTableEntry = localEntry->typeInfo->type;
        node->tupleType = localEntry->typeInfo->tupleType;
        node->typeInfo = localEntry->typeInfo;
    } else if (globalEntry) {
        node->type = globalEntry->typeInfo->kind;
        node->gSymbolTableEntry = globalEntry;
        node->lSymbolTableEntry = NULL;
        node->typeTableEntry = globalEntry->typeInfo->type;
        node->tupleType = globalEntry->typeInfo->tupleType;
...
struct tnode *createTypeNode(enum Type type) {
    ...
    node->_class = NULL;
    node->typeInfo = createTypeInfo(type, NULL, node->typeTableEntry, node->_class);
...
struct tnode *createBreakPointNode() { return createLeafNode(NODE_BREAK_POINT); }
