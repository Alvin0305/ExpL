...
struct tnode *createNewNode(struct tnode *varNameNode, struct tnode *classNameNode) {
    struct tnode *varUsageNode = createVariableUsageNode(varNameNode->varName);
    struct ClassTable *_class = lookupCT(classNameNode->varName);
    if (!_class) {
        printf("error\n");
    }

    struct tnode *node = createConnectorNode(varUsageNode, classNameNode);
