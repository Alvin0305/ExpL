...
void compilerError(enum Error error, ...) {
    ...
    switch (error) {
        ...
        case E_INVALID_REGISTER:
            fprintf(stderr, "[ERROR]: (%d) Invalid register is getting allocated. Check the code\n", lineNumber);
            break;
        ...
        case E_FUNCTION_REDECLARATION:
            functionName = va_arg(ap, char *);
            fprintf(stderr, "[ERROR]: (%d) Function \"%s\" is redeclarated\n", lineNumber, functionName);
            break;
        ...
        case E_FUNCTION_RETURN_TYPE_MISMATCH:
            ...
            expectedType = va_arg(ap, int);
            givenType = va_arg(ap, int);
            fprintf(stderr, "[ERROR]: Expected return type of function \"%s\" is %s, but got %s\n", functionName,
        ...
        case E_DEREFERENCING_NON_POINTER_VARIABLE:
            variableName = va_arg(ap, char *);
            fprintf(stderr, "[ERROR]: (%d) Variable \"%s\" is not a pointer to dereference\n", lineNumber, variableName);
            break;

        case E_DEREFERENCE_NON_POINTER_TUPLE:
            variableName = va_arg(ap, char *);
            fprintf(stderr, "[ERROR]: Variable \"%s\" is not a pointer tuple to dereference\n", variableName);
            break;
        ...
        case E_MEMBER_ACCESS_ON_NON_SUPPORTED_TYPE:
            ...
            givenType = va_arg(ap, int);
            fprintf(stderr,
                    "[ERROR]: (%d) Accessing a member is supported in TUPLE and User Defined Typed. Used in %s having type %s\n",
                    lineNumber, variableName, dataTypeToString(givenType));
            break;
        ...
        case E_ASSIGN_TYPE_MISMATCH:
            ...
            fprintf(stderr, "[ERROR]: [%d] Assignment type mismatch, %s = %s is not allowed\n", lineNumber,
        ...
        case E_ASSIGN_USER_TYPE_MISMATCH:
            ...
            fprintf(stderr, "[ERROR]: [%d] Assignment type mismatch %s = %s is not allowed\n", lineNumber, leftT->name,
                    rightT->name);
            break;

        case E_CLASS_USED_BEFORE_DECLARATION:
            className = va_arg(ap, char *);
            fprintf(stderr, "[ERROR]: [%d] Class %s is used before it is declared\n", lineNumber, className);
            break;
