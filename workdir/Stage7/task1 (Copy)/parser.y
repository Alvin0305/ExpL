...
%type <classField> classDecl classDeclList classFieldList 
%type <param> methodParamList methodParam
%type <classMethod> classMethodDecl classMethodDeclList
...
%token KW_NULL
%token KW_CLASS KW_ENDCLASS KW_SELF
...
classDef : ID { createNewClass($1->varName); } '{' classDeclBlock classMethodDefBlock '}'
    ;

classDeclBlock : KW_DECL classDeclList classMethodDeclList KW_ENDDECL   { setFieldsToClass($2); setMethodsToClass($3); }
    | KW_DECL KW_ENDDECL
    |
    ;
...
classDeclList : classDeclList classDecl                             { $$ = mergeClassFields($1, $2); }
    | classDecl                                                     { $$ = $1; }
    ;

classMethodDeclList : classMethodDeclList classMethodDecl           { $$ = mergeClassMethods($1, $2); }
    | classMethodDecl                                               { $$ = $1; }
    ;

classDecl : type classFieldList SEMI                                { $$ = $2; }
    ;

classMethodDecl : type ID '(' methodParamList ')' SEMI              { $$ = createNewClassMethod($2->varName, $4); }
    ;

classFieldList : classFieldList COMMA ID                            { $$ = mergeClassFields($1, createNewClassField($3->varName)); }
    | ID                                                            { $$ = createNewClassField($1->varName); }
    ;
...
methodParam : type ID                                               { $$ = createNewParam($1->typeTableEntry, NULL, $2->varName, false); }
    | type MUL ID                                                   { $$ = createNewParam($1->typeTableEntry, NULL, $3->varName, true); }
    ;
...
type : TYPE_INT                                                     { $$ = createTypeNode(INT); setActiveClassFieldType(lookupTT("INT"), NULL); }
    | TYPE_STRING                                                   { $$ = createTypeNode(STRING); setActiveClassFieldType(lookupTT("STRING"), NULL); }
    | KW_TUPLE ID                                                   { $$ = createTupleTypeNode($2); setActiveClassFieldType(lookupTT($2->varName), NULL); }
    | TYPE_VOID                                                     { $$ = createTypeNode(VOID); setActiveClassFieldType(lookupTT("VOID"), NULL); }
    | ID                                                            { $$ = createUserTypeNode($1); setActiveClassFieldType(lookupTT($1->varName), NULL); }
    ;
...
fieldAccess : fieldAccess DOT ID                                    { $$ = createMemberAccessNode($1, $3, ACCESS_DOT); }
    ...
    | KW_SELF DOT ID                                                { $$ = createEmptyNode(); }  // change these 3 later
    | KW_SELF DOT ID '(' argList ')'                                { $$ = createEmptyNode(); }
    | fieldAccess DOT ID '(' argList ')'                            { $$ = createEmptyNode(); }
    ;
