...
void compilerError(enum Error error, ...) {
    ...
    switch (error) {
        ...
        case E_FUNCTION_REDECLARATION:
            functionName = va_arg(ap, char *);
            fprintf(stderr, 
                "[ERROR]: (%d) Function \"%s\" is redeclarated\n", lineNumber, functionName
            );
            break;
        ...
        case E_DEREFERENCING_NON_POINTER_VARIABLE:
            variableName = va_arg(ap, char *);
            fprintf(stderr,
                    "[ERROR]: (%d) Variable \"%s\" is not a pointer to dereference\n",
        ...
        case E_DEREFERENCE_NON_POINTER_TUPLE:
            variableName = va_arg(ap, char *);
            fprintf(stderr,
                    "[ERROR]: Variable \"%s\" is not a pointer tuple to dereference\n",
                    variableName);
            break;
        ...
        case E_ASSIGN_USER_TYPE_MISMATCH:
            ...
            fprintf(stderr,
                "[ERROR]: [%d] Assignment type mismatch %s = %s is not allowed\n",
                lineNumber, leftT->name, rightT->name
            );
            break;
        ...
        case E_CLASS_USED_BEFORE_DECLARATION:
            ...
            fprintf(stderr,
                "[ERROR]: [%d] Class %s is used before it is declared\n",
