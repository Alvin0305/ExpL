...
struct Node *createTupleAccessNode(struct Node *idNode, struct Node *fieldNode) {
    ...
    struct Node *node = createConnectorNode(idUsageNode, fieldNode);
...
struct Node *createTupleAssignmentNode(struct Node *idNode, struct Node *fieldNode, struct Node *exprNode) {
    ...
    if (!isTypeCompatible(createTypeInfo(fieldEntry->type, NULL, lookupTT(dataTypeToString(fieldEntry->type)), NULL),
                          exprNode->typeInfo)) {
    ...
    struct Node *tupleNode = createConnectorNode(idUsageNode, fieldNode);
    tupleNode->nodeType = NODE_TUPLE_ACCESS;

    struct Node *node = createConnectorNode(tupleNode, exprNode);
...
struct Node *createTuplePointerAssignmentNode(struct Node *idNode, struct Node *fieldNode, struct Node *exprNode) {
    ...
    struct Node *tupleNode = createConnectorNode(idNode, fieldNode);
